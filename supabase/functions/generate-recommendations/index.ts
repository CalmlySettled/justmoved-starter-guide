...
serve(async (req) => {
  ...
  try {
    ...
    const { responses, userId, sessionToken, previewMode }: { 
    ...
    // Validation
    if (!responses || !responses.address) {
      throw new Error('Invalid request: address is required');
    }

    // Get user coordinates
    const coordinates = await getCoordinatesFromAddress(responses.address);
    if (!coordinates) {
      throw new Error('Could not get coordinates for the provided address');
    }

    console.log(`Processing recommendations for user ${userId || 'anonymous'} at ${coordinates.lat}, ${coordinates.lng}${previewMode ? ' (PREVIEW MODE)' : ''}`);

    // Round coordinates for better cache efficiency
    const roundedCoords = roundCoordinates(coordinates.lat, coordinates.lng);
    ...
    if (userId) {
      const { data: profile } = await supabase
        .from('profiles')
        .select('display_name, ai_insights')
        .eq('user_id', userId)
        .single();
      userProfile = profile;
    }
    ...
    // Generate cache key that includes preview mode
    const cacheKey = `recommendations_${roundedCoords.lat}_${roundedCoords.lng}_${responses.priorities.sort().join('_')}_${responses.budgetPreference}_${responses.transportationStyle}${userId ? `_user_${userId}` : ''}${previewMode ? '_preview' : ''}`;
    ...
    // Generate recommendations for each priority category
    const allRecommendations: { [key: string]: Business[] } = {};
    
    for (const category of responses.priorities) {
      console.log(`\n=== Processing category: ${category} ===`);
      
      const categoryBusinesses = await searchBusinessesForCategory(category, coordinates.lat, coordinates.lng, sessionToken);
      
      if (categoryBusinesses.length > 0) {
        // Apply preview mode limit - only return top 2 businesses for unauthenticated users
        const businesses = previewMode ? categoryBusinesses.slice(0, 2) : categoryBusinesses;
        allRecommendations[category] = businesses;
        
        console.log(`→ Added ${businesses.length} businesses for ${category}${previewMode ? ' (preview mode)' : ''}`);
      }
    }

    // Apply AI scoring if enabled and user profile exists
    if (Math.random() < AI_RECOMMENDATION_PERCENTAGE && userProfile?.ai_insights) {
      console.log('\n🤖 Applying AI recommendation scoring...');
      await applyAIRecommendationScoring(allRecommendations, responses, userProfile);
    }
    ...
    console.log(`\n✅ Generated ${Object.keys(allRecommendations).length} categories with total ${Object.values(allRecommendations).flat().length} businesses${previewMode ? ' (preview mode)' : ''}`);

    return new Response(JSON.stringify(allRecommendations), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
