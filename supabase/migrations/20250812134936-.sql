...
-- Add coordinate privacy to prevent individual user tracking
...
-- Simple hash function for coordinate privacy (using built-in functions)
...
AS $$
BEGIN
  -- Simple hash using built-in functions for privacy
  -- Rounds coordinates and adds user salt to prevent individual tracking
  RETURN md5(
